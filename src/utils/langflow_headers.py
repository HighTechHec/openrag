...
async def add_provider_credentials_to_headers(
    ...
    Args:
        ...
        selected_provider: Optional provider name (kept for backwards compatibility).
            Credentials are added for all configured providers to support mixed
            provider/model datasets.
    """
    _ = (selected_provider or "").lower().strip() or None
...
async def build_mcp_global_vars_from_config(config, flows_service=None) -> Dict[str, str]:
    ...
    # Include all configured provider credentials.
    # This supports mixed-ingestion datasets where different documents may have
    # been embedded with different providers/models over time.
    if config.providers.openai.api_key:
