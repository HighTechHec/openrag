...
OpenRAG passes flow runtime values through Langflow global-variable headers (`X-Langflow-Global-Var-*`) on each run request.
This includes request-scoped values such as ownership metadata, file context, chunking settings, and query filters.
